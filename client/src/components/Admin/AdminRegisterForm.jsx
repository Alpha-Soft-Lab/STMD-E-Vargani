    ...
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <div className="relative">
        <input
          ...
          required
          className="w-full px-4 py-2 bg-slate-50 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-1 transition pr-10"
        />
        <span
          onClick={() => setShowPassword(!showPassword)}
          className="absolute inset-y-0 right-3 flex items-center cursor-pointer text-gray-400 hover:text-green-600 transition"
        >
          {showPassword ? (
            <svg
